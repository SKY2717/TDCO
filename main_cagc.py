# --------------------------------------------------------
# MTLoRA with CAGC (Conflict-Aware Gradient Consensus)
    ...
    for epoch in range(config.TRAIN.EPOCHS):
        ...
        if cagc_processor is not None:
            train_one_epoch_cagc(config, model, criterion, data_loader_train, optimizer, epoch, mixup_fn, lr_scheduler,
                            loss_scaler, cagc_processor=cagc_processor, teacher=teacher)
        else:
...
def train_one_epoch(config, model, criterion, data_loader, optimizer, epoch, mixup_fn, lr_scheduler, loss_scaler, task=None, teacher=None):
    """Original training function without CAGC"""
    model.train()
    optimizer.zero_grad()
...
def train_one_epoch_cagc(config, model, criterion, data_loader, optimizer, epoch, mixup_fn, 
                        lr_scheduler, loss_scaler, cagc_processor=None, teacher=None):
    """
    CAGC-enabled training function.
